...
public class Packet {
    ...
    static public void answerToPacket(Packet packet, MessageType type, String text,
                                      ConcurrentLinkedQueue<Packet> packetsToSend, int ttl, String dstName) {
    ...
    static public void spreadPacket(Packet packet, ConcurrentLinkedQueue<Packet> packetsToSend, int ttl,
                                    TreeNode node, boolean onlyChildren) {
        ...
        if (node.hasParent() && !onlyChildren) {
            if (!src.equals(node.getParent())) {
                messageToSpread = new Message(packet.getMessage().getMessageType(), packet.getMessage().getName(),
                        packet.getMessage().getMessageText(), UUID.randomUUID());
