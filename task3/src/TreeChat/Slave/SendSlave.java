...
public class SendSlave implements Runnable {
    ...
    public void run() {
        ...
        if (node.hasParent()) {
            Message adoptMessage = new Message(MessageType.ADOPT_CHILD_MESSAGE, node.getName(),
                    "127.0.0.1:" + socket.getLocalPort(), UUID.randomUUID());
        ...
        while (true) {
            try {
                ...
                if (packet != null) {
                    ...
                    if (packet.getMessage().getMessageType() != MessageType.PING_MESSAGE) {
                        System.out.println("Msg type: " + packet.getMessage().getMessageType());
                        System.out.println("Msg ttl: " + packet.getTtl());
                        System.out.println("Sending msg: " + packet.getMessage().getMessageText() + " to " +
